...
public sealed partial class ScenarioConditionForm : ViewModelBase
{
    ...
    public void Load(SimulationRequest request)
    {
        ...
        SelectedStrategyMode = request.NewCashFlowAllocationMode is { } mode ? StrategyModes.First(x => x.Value == mode) : SelectedStrategyMode;
        SelectedStrategySalaryDate = request.EffectivePeriodDate is { } date ? StrategySalaryDates.FirstOrDefault(x => x.Value == date) : SelectedStrategySalaryDate;
