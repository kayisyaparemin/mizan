...
public partial class SimulationViewModel
{
    private void Populate(SimulationResult result)
    {
        ...
        AssignmentModeText = AssignmentModeLabel(
            result.Scenario[0].CashFlowAllocationMode);
        ...
        IReadOnlyList<CashFlowPeriodProjection> baseline)
    {
        ...
        AssignmentModeText = AssignmentModeLabel(
            baseline[0].CashFlowAllocationMode);
        LoanImpacts.Clear();
    ...
    private string BuildApplyConfirmation(SimulationRequest request)
    {
        ...
        var detail = request.Type switch
        {
            ...
            SimulationScenarioType.PaymentStrategyChange =>
                $"Başlangıç dönemi: {LongDate(request.EffectivePeriodDate)}",
            SimulationScenarioType.LoanEarlyClosure =>
    ...
    private string ConditionSummaryText(SimulationRequest request)
    {
        ...
        return request.Type switch
        {
            ...
            SimulationScenarioType.SalaryChange =>
                $"{amount} • Yeni gelir",
            SimulationScenarioType.PaymentStrategyChange =>
                $"{StrategyModeLabel(request.NewCashFlowAllocationMode)} • {LongDate(request.EffectivePeriodDate)} dönemi",
            SimulationScenarioType.CreditCardPaymentMode =>
    ...
    private static string StrategyModeLabel(CashFlowAllocationMode? mode) =>
    ...
    private static string TargetPeriodText(CashFlowPeriod period) =>
        period.Start.ToString("MMMM yyyy", TurkishCulture);

    private static string AssignmentModeLabel(CashFlowAllocationMode mode) =>
        mode == CashFlowAllocationMode.PreviousPeriod
