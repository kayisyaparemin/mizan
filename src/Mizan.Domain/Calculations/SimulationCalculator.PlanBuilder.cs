...
public sealed partial class SimulationCalculator
{
    ...
    private static (DateOnly Date, int Type, string Name, decimal Amount, Guid Id)
        SortKey(SimulationRequest request) => (
            request.EffectivePeriodDate ?? request.StartDate,
            (int)request.Type,
            request.Name.Trim(),
            request.Amount,
            request.ScenarioId);
    ...
    private static FinancialPlan AddPaymentStrategy(
        ...
        SimulationRequest request)
    {
        var effectiveDate = request.EffectivePeriodDate ?? request.StartDate;
        if (CalendarRules.ResolveDay(
                effectiveDate.Year,
                effectiveDate.Month,
                plan.Settings.IncomeDay) != effectiveDate)
        {
            throw new InvalidOperationException(
                "Düzen değişikliği yalnızca bir dönem tarihinde başlayabilir.");
        }

        var mode = request.NewCashFlowAllocationMode ??
                   throw new InvalidOperationException(
                       "Yeni dönem kullanım düzeni seçilmelidir.");
        return plan with
        {
            PaymentAssignmentStrategies = plan.PaymentAssignmentStrategies
                .Where(x => x.EffectiveFromPeriodDate != effectiveDate)
                .Append(new CashFlowAllocationStrategy
                {
                    Id = request.ScenarioId,
                    Mode = mode,
                    EffectiveFromPeriodDate = effectiveDate,
