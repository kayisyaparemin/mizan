    ...
    LoanPaymentScheduleBuilder? loanScheduleBuilder = null)
{
        ...
        DateOnly? firstSalaryDate = null)
    {
        ...
        var firstNegativeCapacity = scenario
            .FirstOrDefault(x => x.EstimatedSurplus < 0m);
        var firstNegativeSavings = scenario
            .FirstOrDefault(x => x.EndingProjectedBalance < 0m);
        ...
        var recovery = scenario.FirstOrDefault(x =>
            x.HasCarryOverDeficit && x.EndingProjectedBalance >= 0m);
        ...
        var risk = new SimulationRiskSummary(
            ...
            scenario.Min(x => x.EstimatedSurplus),
            scenario.Min(x => x.EndingProjectedBalance),
            lowest.Period,
        ...
        decimal endingDifference)
    {
        ...
        if (risk.FirstNegativeProjectedBalancePeriod is CashFlowPeriod negative)
        {
            parts.Add(
                $"Bu plan {PeriodMonth(negative)} döneminde finansman açığı oluşturuyor.");
            parts.Add(risk.RecoveryPeriod is CashFlowPeriod recovered
        ...
        {
            parts.Add(risk.RecoveryPeriod is CashFlowPeriod openingRecovery
    ...
    private static string PeriodMonth(CashFlowPeriod period) =>
