    ...
    public static async Task SeedAsync(SQLiteAsyncConnection database)
    {
        ...
        foreach (var (id, dueDate, amount) in eminevim)
        {
            await database.InsertOrReplaceAsync(new PaymentInstallmentRow
            {
                ...
                DueDate = SqliteMizanStore.FormatDate(dueDate),
                Amount = amount
            });
        }
        ...
        await database.InsertOrReplaceAsync(SqliteMizanStore.ToRow(card));
        await database.InsertOrReplaceAsync(SqliteMizanStore.ToRow(
        ...
        foreach (var (id, postingDate, amount) in cardCharges)
        {
            await database.InsertOrReplaceAsync(new CardInstallmentRow
            {
                ...
                DueDate = SqliteMizanStore.FormatDate(postingDate),
                Amount = amount
            });
        }

        await database.InsertOrReplaceAsync(
            new CashFlowAllocationStrategyRow
            {
                Id = InitialAssignmentStrategyId.ToString("D"),
                Mode = (int)CashFlowAllocationMode.UpcomingPeriod,
                EffectiveFromPeriodDate = SqliteMizanStore.FormatDate(
