...
/// <summary>
/// Uygulamanın gördüğü tek <see cref="IMizanStore"/>. Her çağrıyı o an
...
/// </remarks>
public sealed class ProfileScopedMizanStore(
    Func<Guid, IMizanStore> storeFactory) :
    IMizanStore,
    IProfileStoreSwitch,
    IAsyncDisposable
{
    private IMizanStore? _current;
    ...
    private IMizanStore Current =>
        _current ?? throw new InvalidOperationException(
            "Açık bir profil yok. Devam etmek için bir profil seç.");

    private static async Task DisposeStoreAsync(IMizanStore? store)
    {
    ...
    public Task<IReadOnlyList<PaymentReminderResponse>> GetPaymentReminderResponsesAsync(
        CancellationToken cancellationToken = default) =>
        Current.GetPaymentReminderResponsesAsync(cancellationToken);

    public Task UpsertPaymentReminderResponsesAsync(
        IReadOnlyList<PaymentReminderResponse> responses,
        CancellationToken cancellationToken = default) =>
        Current.UpsertPaymentReminderResponsesAsync(responses, cancellationToken);

    public Task DeletePaymentReminderResponseAsync(
        string dueKey,
        CancellationToken cancellationToken = default) =>
        Current.DeletePaymentReminderResponseAsync(dueKey, cancellationToken);

    public Task<IReadOnlyList<CashFlowAllocationStrategy>>
        GetPaymentAssignmentStrategiesAsync(
            CancellationToken cancellationToken = default) =>
        Current.GetPaymentAssignmentStrategiesAsync(cancellationToken);

    public Task UpsertCashFlowAllocationStrategyAsync(
        CashFlowAllocationStrategy strategy,
        CancellationToken cancellationToken = default) =>
        Current.UpsertCashFlowAllocationStrategyAsync(strategy, cancellationToken);

    public Task DeleteCashFlowAllocationStrategyAsync(
        Guid id,
        CancellationToken cancellationToken = default) =>
        Current.DeleteCashFlowAllocationStrategyAsync(id, cancellationToken);
