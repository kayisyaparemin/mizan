...
internal static class ObligationValidation
{
    ...
    {
        CalendarRules.ValidateDay(draft.Settings.IncomeDay);
        if (draft.Settings.MonthlyVariableExpenseAllowance < 0m)
        {
        ...
        if (!Enum.IsDefined(draft.InitialCashFlowAllocationMode))
        {
            throw new InvalidOperationException(
