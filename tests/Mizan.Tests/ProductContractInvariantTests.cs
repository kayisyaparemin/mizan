...
public sealed class ProductContractInvariantTests
{
    ...
    public async Task P6_DashboardAndTwelvePeriodShareTheSameProjection()
    {
        await WithSeededService(async service =>
        {
            ...
            Assert.Equal(
                periods[0].EndingProjectedBalance,
                dashboard.CurrentPeriod.EndingProjectedBalance);
            Assert.Equal(
                periods[0].MandatoryOutflow,
                dashboard.CurrentPeriod.MandatoryOutflow);
            Assert.Equal(
                periods[^1].EndingProjectedBalance,
                dashboard.TwelvePeriodEndingProjectedBalance);
        });
    }
    ...
    public async Task I7_CarryOverDeficitIsNotSubtractedTwice()
    {
        await WithSeededService(async service =>
        {
            ...
            {
                Assert.Equal(
                    period.OpeningProjectedBalance +
                    period.CurrentPeriodNetContribution,
                    period.EndingProjectedBalanceBeforeDeficitInterest);
            }
        });
    }
    ...
    public async Task I14_MidPeriodObservationDoesNotAdvanceTheSnapshotChain()
    {
        await WithSeededService(async service =>
        {
            ...
            Assert.Equal(
                before.PlannedEndingBalance,
                after.PlannedEndingBalance);
        });
    }
    ...
    public async Task I16_ObservationDoesNotLeakIntoTheProjection()
    {
        await WithSeededService(async service =>
        {
            ...
            Assert.Equal(
                beforeEndings,
                after.Select(x => x.EndingProjectedBalance));
        });
    }

    private static async Task WithSeededService(
        Func<Application.Services.MizanService, Task> test)
    {
        ...
        try
        {
            await using var store = new SqliteMizanStore(path, true, Init);
