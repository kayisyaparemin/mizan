...
/// <c>Mizan.App</c>'e referans vermediği için bu kurallar
...
public sealed class UiPatchSourceTests
{
    ...
    public void CurrentBalance_IsNoLongerOfferedInSettings()
    {
        ...
        Assert.DoesNotContain("ProjectionOpeningBalance", settings);
        // Yaşam gideri ve faiz oranları Ayarlar'da kalır.
        Assert.Contains("MonthlyVariableExpenseAllowance", settings);
    ...
    public void MainPage_SaysTheSameThingOnce()
    {
        ...
        Assert.DoesNotContain("Text=\"{Binding endingBalance}\"", page);
    ...
    private static string PageSource(string fileName) => File.ReadAllText(
        Path.Combine(
            RepositoryRoot(), "src", "Mizan.App", "Pages", fileName));

    private static string ViewModelSource(string fileName) => File.ReadAllText(
        Path.Combine(
            RepositoryRoot(), "src", "Mizan.App", "ViewModels", fileName));
