...
public sealed class FinancialProjectionTests
{
    ...
    public void FirstFourCanonicalSalaryPeriods_AreExact()
    {
        ...
        Assert.Equal(10_198.85m, rows[0].EndingProjectedBalance);
        Assert.Equal(23_342.79m, rows[1].EndingProjectedBalance);
        Assert.Equal(13_591.52m, rows[2].EndingProjectedBalance);
    ...
    public void OneTimeIncome_BetweenAnchorAndFirstSalary_LandsInFirstPeriod()
    {
        ...
        Assert.Equal(
            ...
            rows[0].TotalIncome);
        Assert.Equal(
            baseline[0].EndingProjectedBalance + 150_000m,
            rows[0].EndingProjectedBalance);
        Assert.Contains(
            rows[0].IncomeItems,
    ...
    public void OneTimeIncome_BeforeAnchor_StaysOutOfProjection()
    {
        ...
        Assert.Equal(
            baseline[0].EndingProjectedBalance,
            rows[0].EndingProjectedBalance);
    }
    ...
    public void CanonicalUpcomingPlan_ExposesPreSalaryObligationsSeparately()
    {
        ...
        Assert.Equal(2, result.allocationPlan.PreFirstPeriodObligations.Count);
        Assert.Equal(
            53_799.54m,
            result.allocationPlan.PreFirstPeriodObligations.Sum(x => x.Amount));
        Assert.Equal(
            result.allocationPlan.EligiblePaymentCount,
            result.allocationPlan.AssignedExactlyOnceCount);
        Assert.Equal(0, result.allocationPlan.UnassignedPaymentCount);
        Assert.Equal(0, result.allocationPlan.DuplicateAssignedCount);
    }
    ...
    [Fact]
    public void VariableExpenseAllowance_ProducesNegativeSavingsCapacity()
    {
        ...
        Assert.Equal(-5_000m, row.EstimatedSurplus);
    ...
    public void ProjectedSavings_IsCumulativeFromConfiguredStart()
    {
        var plan = BasicPlan(50_000m) with
        {
            Settings = new UserSettings
            {
                IncomeDay = 10,
                MonthlyVariableExpenseAllowance = 30_000m,
                ProjectionOpeningBalance = 100_000m
            }
        };
        ...
        Assert.Equal(100_000m, rows[0].OpeningProjectedBalance);
        Assert.Equal(120_000m, rows[0].EndingProjectedBalance);
        Assert.Equal(120_000m, rows[1].OpeningProjectedBalance);
        Assert.Equal(160_000m, rows[2].EndingProjectedBalance);
    }
    ...
    public void CarryOverDeficit_IsVisibleWithoutBeingCountedTwice()
    {
        ...
        Assert.Equal(-25_987m,
            rows[0].EndingProjectedBalanceBeforeDeficitInterest);
        Assert.Equal(1_299.35m, rows[0].DeficitFinancingInterest);
        Assert.Equal(-27_286.35m, rows[0].EndingProjectedBalance);
        Assert.Equal(-27_286.35m, current.OpeningProjectedBalance);
        ...
        Assert.Equal(30_000m, current.VariableExpenseAllowance);
        Assert.Equal(34_957m, current.EstimatedSurplus);
        Assert.Equal(7_670.65m,
            current.EndingProjectedBalanceBeforeDeficitInterest);
        Assert.Equal(0m, current.DeficitFinancingInterest);
        Assert.Equal(7_670.65m, current.EndingProjectedBalance);
    ...
    public void DeficitContinuesIntoNextOpeningWithoutClampOrFakeObligation()
    {
        ...
        Assert.Equal(-5_250m, rows[0].EndingProjectedBalance);
        Assert.Equal(-5_250m, rows[1].OpeningProjectedBalance);
    ...
    public void DeficitRecovery_ProducesPositiveEndingWithoutDoubleCount()
    {
        ...
        Assert.Equal(15_000m, row.EndingProjectedBalance);
    ...
    public void PositiveOpeningSavings_DoesNotCreateCarryOverDeficit()
    {
        ...
        Assert.Equal(30_000m, row.EndingProjectedBalance);
    }
    ...
    public void LargeCashExpense_ImpactsItsPeriodAndAllFutureSavings()
    {
        ...
        Assert.Equal(
            baselineRows[0].EstimatedSurplus - 350_000m,
            scenarioRows[0].EstimatedSurplus);
        Assert.Equal(-330_000m,
            scenarioRows[0].EndingProjectedBalanceBeforeDeficitInterest);
        Assert.Equal(16_500m,
            scenarioRows[0].DeficitFinancingInterest);
        Assert.Equal(-346_500m,
            scenarioRows[0].EndingProjectedBalance);
        Assert.Equal(-338_966.25m,
            scenarioRows[2].EndingProjectedBalance);
    }
    ...
    public void JanuarySalaryIncrease_StartsWithJanuarySalaryPeriod()
    {
        ...
        Assert.Equal(115_000m, december.PrimaryIncome);
        Assert.Equal(132_250m, january.PrimaryIncome);
    }
    ...
    public void PreviousFullStatement_AssignsSeptemberStatementToSeptemberSalary()
    {
        ...
        Assert.Equal(-27_747.00m, row.EstimatedSurplus);
        Assert.Equal(1_387.35m, row.DeficitFinancingInterest);
        Assert.Equal(-29_134.35m, row.EndingProjectedBalance);
    }
    ...
    public void DeficitInterest_CompoundsAndStopsOnRecovery()
    {
        var continuing = BasicPlan(40_000m) with
        {
            Settings = new UserSettings
            {
                IncomeDay = 10,
                MonthlyVariableExpenseAllowance = 30_000m,
                ProjectionOpeningBalance = -26_250m,
                ProjectionAnchorDate = new DateOnly(2026, 8, 20),
        ...
        Assert.Equal(-17_062.50m, row.EndingProjectedBalance);
        ...
        Assert.Equal(13_750m,
            recovery.EndingProjectedBalanceBeforeDeficitInterest);
        Assert.Equal(0m, recovery.DeficitFinancingInterest);
        Assert.Equal(13_750m, recovery.EndingProjectedBalance);
    }
    ...
    public void ZeroDeficitInterestRate_LeavesNegativeEndingUnchanged()
    {
        ...
        Assert.Equal(-25_000m, row.EndingProjectedBalance);
    }
    ...
    public void CardAndDeficitInterest_RemainSeparateAndTotalExactly()
    {
        ...
        Assert.Equal(-27_000m,
            result.Periods[0].EstimatedSurplus);
        Assert.Equal(-27_000m,
            result.Periods[0].EndingProjectedBalanceBeforeDeficitInterest);
        ...
        decimal savings)
    {
        ...
        Assert.Equal(living, row.VariableExpenseAllowance);
        Assert.Equal(savings, row.EstimatedSurplus);
    }
}
