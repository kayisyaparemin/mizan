...
public sealed class PeriodHistoryTests
{
    ...
    public void Freeze_KeepsOnlyWindowActivity_AndEndingMatchesHandCalculation()
    {
        ...
        Assert.Equal(FirstPlannedLiving, frozen.PlannedVariableExpenseAllowance);
        Assert.Equal(0m, frozen.PlannedDeficitInterest);
        Assert.Equal(FirstPlannedEnding, frozen.PlannedEndingBalance);
    ...
    public void Freeze_DeficitInterest_RoundsMidpointAwayFromZero()
    {
        ...
        Assert.Equal(-12_962.99m, frozen.PlannedEndingBalance);
    }
    ...
    public void Freeze_PositiveOrZeroEnding_HasNoDeficitInterest()
    {
        ...
        Assert.Equal(0m, zero.PlannedEndingBalance);
        ...
        Assert.Equal(-0.01m, oneKuruşShort.PlannedEndingBalance);
    }
        ...
        string expectedBudget)
    {
        ...
        var plan = SimplePlan() with
        {
            ...
            {
                MonthlyVariableExpenseAllowance = Money(monthlyBudget)
            }
        };

        var frozen = Freeze(plan, Snapshot(date, 0m, IncomeDay));

        Assert.Equal(
            DateOnly.Parse(expectedReviewDate, CultureInfo.InvariantCulture),
            frozen.SettlementAvailableFrom);
        Assert.Equal(frozen.SettlementAvailableFrom, frozen.PeriodEnd);
        Assert.Equal(Money(expectedBudget), frozen.PlannedVariableExpenseAllowance);
    }
    ...
    public void Comparison_Summary_SaysBelowPlan_WithTurkishFormat_OnEnglishDevice()
    {
        ...
        try
        {
            var actual = ComparisonActual() with { ConfirmedEndingBalance = 103_642.61m };
    ...
    {
        var actual = ComparisonActual() with { ConfirmedEndingBalance = 104_877.17m };
    ...
    public async Task ReviewContext_SuggestsPlannedEnding_BeforeAnyActualIsEntered()
    {
        await WithSimpleStore(async store =>
        {
            ...
            Assert.Equal(context.OriginalPlan.PlannedEndingBalance, context.SuggestedStartingSavings);
        });
    }
    ...
    public async Task Finalize_RecordsEveryActualAmount_AndDerivesEndingByHand()
    {
        await WithSimpleStore(async store =>
        {
            ...
            Assert.Equal(derived, actual.DerivedEndingBalance);
            Assert.Equal(111_000m, actual.ConfirmedEndingBalance);
            ...
            Assert.Equal(111_000m, stored.Actual.ConfirmedEndingBalance);
    ...
    public async Task Finalize_OmittedPaymentDraft_CountsAsPaidOnPlannedDateAndAmount()
    {
        await WithSimpleStore(async store =>
        {
            ...
            Assert.Equal(FirstPlannedEnding, result.Actual.DerivedEndingBalance);
        ...
        bool accepted)
    {
        ...
        await WithSimpleStore(async store =>
        {
            ...
            if (accepted)
            {
                ...
                Assert.Equal(FirstPlannedEnding - 100m, result.Actual.DerivedEndingBalance);
            }
            else
            {
    ...
    public async Task Finalize_NewSnapshotAndPlan_StartOnCheckpointWithConfirmedAmount()
    {
        await WithSimpleStore(async store =>
        {
            ...
            Assert.Equal(SecondCheckpoint, snapshot.NextSettlementDate);
            Assert.Equal(95_000m, snapshot.ProjectionOpeningBalance);
            ...
            Assert.Equal(SecondCheckpoint, plan.SettlementAvailableFrom);
            Assert.Equal(snapshot.Id, plan.FinancialSnapshotId);
            Assert.Equal(95_000m, plan.OpeningBalance);
            ...
            Assert.Equal(30_000m, plan.PlannedVariableExpenseAllowance);
            Assert.Equal(142_000m, plan.PlannedEndingBalance);
            Assert.Equal(
                new[]
                {
            ...
            Assert.Equal(95_000m, future[0].OpeningProjectedBalance);
    ...
    public async Task UnpaidObligations_AppearInNextFrozenPlan_AndCanBeClosedThere()
    {
        ...
        await WithSimpleStore(async store =>
        {
            ...
            Assert.Equal(132_177.42m, next.OpeningBalance);
            // 132.177,42 + 103.000 − 35.000 − 13.000 − 30.000
            Assert.Equal(157_177.42m, next.PlannedEndingBalance);
    ...
    public async Task ThreeConsecutivePeriods_ChainDatesOpeningsAndHistoryOrder()
    {
        await WithSimpleStore(async store =>
        {
            ...
            for (var index = 0; index < checkpoints.Length; index++)
            {
                ...
                Assert.Equal(expectedOpenings[index], context.OriginalPlan.OpeningBalance);
                Assert.Equal(expectedOpenings[index], context.Snapshot.ProjectionOpeningBalance);
            ...
            Assert.Equal(
                ...
                history.Select(x => x.Actual.ConfirmedEndingBalance));
            ...
            Assert.Equal(new DateOnly(2026, 12, 10), current.NextSettlementDate);

            var summary = await last.GetHistorySummaryAsync();
            Assert.NotNull(summary);
            Assert.Equal(3, summary.PeriodCount);
            Assert.Equal(history.Sum(x => x.Comparison.PlannedEndingBalance), summary.Planned);
    ...
    public async Task SalaryDay31_ChainsThroughFebruaryAndThirtyDayMonths()
    {
        var plan = SimplePlan() with
        {
            ...
            [
                new CashFlowAllocationStrategy
                {
                    ...
                    EffectiveFromPeriodDate = new DateOnly(2027, 1, 31),
        ...
        await WithStore(plan, new DateOnly(2027, 2, 5), async store =>
        {
            ...
            for (var index = 0; index < checkpoints.Length; index++)
            {
                ...
                Assert.Equal(expectedStarts[index], context.OriginalPlan.PeriodStart);
                Assert.Equal(checkpoints[index], context.OriginalPlan.PeriodEnd);
                Assert.Equal(expectedBudgets[index], context.OriginalPlan.PlannedVariableExpenseAllowance);
                ...
                Assert.Equal(
                    context.OriginalPlan.OpeningBalance + 100_000m - expectedBudgets[index],
            ...
            Assert.Equal(new DateOnly(2027, 5, 31), current.NextSettlementDate);
            Assert.Equal(10_000m + 300_000m - 84_642.86m, current.ProjectionOpeningBalance);
        });
    }
    ...
    public async Task LateFinalization_StampsCheckpointNotToday_AndNextReviewKeepsCalendar()
    {
        await WithSimpleStore(async store =>
        {
            ...
            Assert.Equal(SecondCheckpoint, result.NewSnapshot.NextSettlementDate);
    ...
    {
        var source = new FinancialSnapshot { SnapshotDate = Anchor, ProjectionOpeningBalance = 50_000m };
        ...
        var plan = new PeriodPlanSnapshot
        {
            ...
            SettlementAvailableFrom = FirstCheckpoint,
            PlannedEndingBalance = 1_000m
        };
        ...
        Assert.Equal(3_000m, period.Comparison.PlannedEndingBalance);
    ...
    public async Task HistoryQuery_SameCreatedAt_BreaksTieByRevisionNumber()
    {
        ...
        var service = new HistoryQueryService(
            FakeHistoryStore(new FinancialHistoryData(
                ...
                [plan],
                [
                    new PeriodPlanRevision { PeriodPlanSnapshotId = plan.Id, RevisionNumber = 7, CreatedAtUtc = createdAt, PlannedEndingBalance = 7m },
                    new PeriodPlanRevision { PeriodPlanSnapshotId = plan.Id, RevisionNumber = 2, CreatedAtUtc = createdAt, PlannedEndingBalance = 2m }
                ],
                [
                    new PeriodActual
                    {
        ...
        Assert.Equal(7m, period.Comparison.PlannedEndingBalance);
    }
    ...
    public async Task HistoryQuery_RecentSummary_TakesNewestPeriodsOnly()
    {
        ...
        foreach (var item in periods)
        {
            ...
            var plan = new PeriodPlanSnapshot
            {
                ...
                SettlementAvailableFrom = item.Start.AddMonths(1),
                PlannedEndingBalance = item.Planned
            };
            ...
            actuals.Add(new PeriodActual
            {
                ...
                ResultFinancialSnapshotId = result.Id,
                PeriodStart = item.Start,
                ConfirmedEndingBalance = item.Actual
            });
        }
    ...
    private static FinancialPlan SimplePlan()
    {
        ...
        return new FinancialPlan
        {
            ...
            [
                ...
                {
                    Mode = CashFlowAllocationMode.UpcomingPeriod,
                    EffectiveFromPeriodDate = FirstCheckpoint,
    ...
    private static PeriodPlanSnapshot Freeze(FinancialPlan source, FinancialSnapshot snapshot)
    {
        var plan = source with
        {
            ...
            PaymentAssignmentStrategies = source.PaymentAssignmentStrategies
                .Select(x => x with
                {
                    EffectiveFromPeriodDate = new CashFlowPeriodCalculator()
                        .GetPeriod(snapshot.SnapshotDate, snapshot.IncomeDay)
                        .Start
                })
                .ToArray()
        };
        return new PeriodPlanSnapshotService(
            ...
            new IncomeResolver()).Freeze(plan, snapshot, snapshot.CreatedAtUtc);
    }

    private static FinancialSnapshot Snapshot(
        DateOnly date,
        decimal startingSavings,
        int IncomeDay = 10) => new()
    {
        SnapshotDate = date,
        ProjectionAnchorDate = date,
        NextSettlementDate = new CashFlowPeriodCalculator().GetNextSettlementDate(date, IncomeDay),
    ...
    private static IMizanStore FakeHistoryStore(FinancialHistoryData history)
    {
        var store = DispatchProxy.Create<IMizanStore, HistoryOnlyStore>();
    ...
    public class HistoryOnlyStore : DispatchProxy
    {
        ...
        protected override object? Invoke(MethodInfo? targetMethod, object?[]? args) =>
            targetMethod?.Name == nameof(IMizanStore.GetFinancialHistoryAsync)
                ? Task.FromResult(History)
                : throw new NotSupportedException(targetMethod?.Name);
    }

    private static Task WithSimpleStore(Func<SqliteMizanStore, Task> test) =>
        ...
        Func<SqliteMizanStore, Task> test)
    {
        ...
        try
        {
            await using var store = new SqliteMizanStore(path, true, today);
            ...
            foreach (var item in plan.PaymentAssignmentStrategies)
                await store.UpsertCashFlowAllocationStrategyAsync(item);
