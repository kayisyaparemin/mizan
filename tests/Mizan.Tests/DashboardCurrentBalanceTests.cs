...
public sealed class DashboardCurrentBalanceTests
{
    ...
    public async Task ObservingMidPeriod_LeavesTheFrozenPlanUntouched()
    {
        await WithSeededStore(async store =>
        {
            ...
            Assert.Equal(
                planBefore.PlannedVariableExpenseAllowance,
                planAfter.PlannedVariableExpenseAllowance);
            Assert.Equal(
    ...
    public async Task ObservingMidPeriod_DoesNotCreateASnapshot()
    {
        await WithSeededStore(async store =>
        {
            ...
            Assert.Equal(
                snapshotBefore.ProjectionOpeningBalance,
                snapshotAfter.ProjectionOpeningBalance);
        });
    }
    ...
    public async Task ObservingMidPeriod_DoesNotMoveTheReviewCheckpoint()
    {
        ...
        {
            var before = await store.GetFinancialHistoryAsync();
            var reviewBefore = Assert.Single(before.Plans).SettlementAvailableFrom;
            ...
            Assert.Equal(
                reviewBefore,
                Assert.Single(after.Plans).SettlementAvailableFrom);
    ...
    public async Task WithoutAnObservation_ThereIsNoTrajectory()
    {
        await WithSeededStore(async store =>
        {
            ...
            Assert.NotEqual(0m, progress.PlannedEndingBalance);
        });
    }
    ...
    public async Task Progress_ComesFromTheFrozenPlanWindow()
    {
        await WithSeededStore(async store =>
        {
            ...
            Assert.Equal(plan.PlannedEndingBalance, progress.PlannedEndingBalance);
