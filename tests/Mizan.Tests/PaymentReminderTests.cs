...
public sealed class PaymentReminderTests
{
    ...
    public async Task Mode_DefaultsToOff_PersistsAcrossRestart_AndIsNotPartOfFinanceSettings()
    {
        ...
        try
        {
            await using (var store = new SqliteMizanStore(path, true, new DateOnly(2026, 8, 20)))
            {
                ...
                await store.SaveSettingsAsync(settings with { MonthlyVariableExpenseAllowance = 31_000m });

                Assert.Equal(PaymentReminderMode.Aggressive, await service.GetPaymentReminderModeAsync());
                Assert.Equal(settings with { MonthlyVariableExpenseAllowance = 31_000m }, await store.GetSettingsAsync());
            }

            await using (var reopened = new SqliteMizanStore(path, true, new DateOnly(2026, 8, 20)))
            {
    ...
    public async Task UpgradeFromSchema15_AddsTheColumnAsOff()
    {
        ...
        try
        {
            ...
            await using var upgraded = new SqliteMizanStore(path, true, new DateOnly(2026, 8, 20));
            ...
            Assert.Equal(17, SqliteMizanStore.CurrentSchemaVersion);
        }
        finally
        {
            DeleteDatabase(path);
    ...
    private static async Task WithCanonical(
        DateOnly today,
        Func<SqliteMizanStore, MizanService, Task> test)
    {
        var path = TempPath();
        try
        {
