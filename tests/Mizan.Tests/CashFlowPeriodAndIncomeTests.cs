...
public sealed class CashFlowPeriodAndIncomeTests
{
    private readonly CashFlowPeriodCalculator _periods = new();
        ...
        int expectedDay)
    {
        Assert.Equal(
            ...
            CalendarRules.ResolveDay(year, month, IncomeDay));
    }
    ...
    public void NextSettlementDate_IsFirstResolvedSalaryDateStrictlyAfterSnapshot(
        int snapshotYear,
        int snapshotMonth,
        int snapshotDay,
        int IncomeDay,
        ...
        int expectedDay)
    {
        ...
        var result = _periods.GetNextSettlementDate(snapshot, IncomeDay);
    ...
    [Fact]
    public void IncomeResolver_UsesLatestRecordEffectiveAtPeriodStart()
    {
        var salaries = new[]
        {
    ...
    public void OneTimeIncome_IsAssignedByExactDate()
    {
        ...
        Assert.Equal(100_000m, result.PrimaryIncome);
    ...
    {
        var calculator = new IncomeProjectionCalculator(new IncomeResolver());
        var period = new CashFlowPeriod(
            new DateOnly(2027, 3, 10),
            new DateOnly(2027, 4, 10));
    ...
    public void TargetAmount_AlreadyReachedUsesOpeningSituation()
    {
        var rows = new[]
        {
            ...
            {
                OpeningProjectedBalance = 320_000m
            }
        };
    ...
    private static CashFlowPeriodProjection Projection(
        int index,
        decimal ending) => new(
