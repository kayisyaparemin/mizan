...
public sealed class PersistenceAndSeedTests
{
    ...
    public async Task DevelopmentSeed_ContainsCanonicalRecords()
    {
        await WithStore(true, async store =>
        {
            ...
            Assert.Equal(10, plan.Settings.IncomeDay);
            Assert.Equal(30_000m, plan.Settings.MonthlyVariableExpenseAllowance);
            Assert.Equal(0m, plan.Settings.ProjectionOpeningBalance);
    ...
    public async Task CanonicalOnboardingSample_MatchesDevelopmentSeedProjection()
    {
        await WithStore(true, async seededStore =>
        {
            ...
            try
            {
                await using var onboardingStore = new SqliteMizanStore(
                    onboardingPath,
                    false,
                    Today);
    ...
    public async Task OnboardingExactPeriodDay_PreviousPeriod_DoesNotCrash()
    {
        await WithStore(false, async store =>
        {
            ...
            await service.InitializeFromOnboardingAsync(
                new OnboardingDraft
                {
                    Settings = new UserSettings
                    {
                        ...
                        ProjectionOpeningBalance = 4_013m,
            ...
            Assert.Equal(new DateOnly(2026, 9, 10),
                strategy.EffectiveFromPeriodDate);
            Assert.Equal(new DateOnly(2026, 9, 10),
    ...
    public async Task DevelopmentSeed_IsIdempotentAcrossReopen()
    {
        ...
        try
        {
            await using (var first = new SqliteMizanStore(
                             path, true, Today))
            {
            ...
            await using (var second = new SqliteMizanStore(
                             path, true, Today))
            {
    ...
    public async Task SettledCard_PersistsKnownNextDatesAndRetiresStatement()
    {
        ...
        {
            Guid cardId;
            await using (var store = new SqliteMizanStore(
                             path,
                             true,
                             Today))
            {
            ...
            await using var reopened = new SqliteMizanStore(
                path,
                true,
                Today);
    ...
    public async Task DevelopmentSeed_UpsertsCanonicalDataIntoExistingDatabase()
    {
        await WithStore(true, async store =>
        {
            ...
            Assert.Equal(CashFlowAllocationMode.UpcomingPeriod, initial.Mode);
            Assert.Equal(new DateOnly(2026, 9, 10),
                initial.EffectiveFromPeriodDate);
        });
    }
        ...
        try
        {
                             ...
                             path, false, Today))
            {
                ...
                var strategy = Assert.Single(
                    ...
                    .PaymentAssignmentStrategies);
                Assert.Equal(CashFlowAllocationMode.PreviousPeriod, strategy.Mode);
                Assert.Equal(new DateOnly(2026, 9, 10),
                    strategy.EffectiveFromPeriodDate);
                Assert.NotNull(await service.GetDashboardAsync());
            }
    ...
    public async Task DevelopmentClear_LeavesValidEmptyState_AndDoesNotReseed()
    {
        await WithStore(true, async store =>
        {
            ...
            var strategy = Assert.Single(
                (await service.GetFinancialPlanAsync())
                .PaymentAssignmentStrategies);
            await service.SaveCashFlowAllocationStrategyAsync(strategy with
            {
            ...
            Assert.Equal(0m, reset.Settings.MonthlyVariableExpenseAllowance);
            Assert.Equal(0m, reset.Settings.ProjectionOpeningBalance);
    ...
    public async Task ProductionEmptyDatabase_IsNotSeeded()
    {
        await WithStore(false, async store =>
        {
            ...
            Assert.Empty(plan.CreditCards);
            Assert.Equal(0m, plan.Settings.MonthlyVariableExpenseAllowance);
            Assert.Equal(default, plan.Settings.ProjectionAnchorDate);
            Assert.Equal(0.05m,
                plan.Settings.CreditCardCarryInterestRate);
            Assert.Equal(0.05m,
    ...
    [Fact]
    public async Task CashFlowAllocationStrategy_PersistsAcrossReopen()
    {
        var path = TempPath();
        try
        {
    ...
    public async Task StrategyPreview_DoesNotMutateHistory_AndFutureRecordCanBeDeleted()
    {
        await WithStore(true, async store =>
        {
            ...
            Assert.Equal(new DateOnly(2026, 12, 10),
                preview.EffectivePeriodDate);

            var future = new CashFlowAllocationStrategy
            {
                Mode = CashFlowAllocationMode.PreviousPeriod,
                EffectiveFromPeriodDate = new DateOnly(2026, 12, 10),
                Note = "Planlı değişiklik"
            };
            await service.SaveCashFlowAllocationStrategyAsync(future);
            Assert.Equal(2, (await service.GetFinancialPlanAsync())
                .PaymentAssignmentStrategies.Count);

            await service.DeleteCashFlowAllocationStrategyAsync(future.Id);
    ...
    public async Task LaterStrategyChanges_InsertEvents_AndPreserveEarlierHistory()
    {
        await WithStore(false, async store =>
        {
            ...
            await service.SaveCashFlowAllocationStrategyAsync(
                ...
                {
                    Mode = CashFlowAllocationMode.PreviousPeriod,
                    EffectiveFromPeriodDate = new DateOnly(2027, 4, 10),
                    Note = "Üçüncü karar"
                });
            ...
            Assert.Collection(
                ...
                {
                    Assert.Equal(new DateOnly(2026, 12, 10),
                        second.EffectiveFromPeriodDate);
                    Assert.Equal(CashFlowAllocationMode.UpcomingPeriod,
                        second.Mode);
                },
                third =>
                {
                    Assert.Equal(new DateOnly(2027, 4, 10),
                        third.EffectiveFromPeriodDate);
                    Assert.Equal(CashFlowAllocationMode.PreviousPeriod,
                        third.Mode);
                });
        });
    }
    ...
    public async Task HistoricalStrategyCorrection_RequiresExplicitConfirmation()
    {
        await WithStore(true, async store =>
        {
            ...
            await Assert.ThrowsAsync<InvalidOperationException>(() =>
                service.SaveCashFlowAllocationStrategyAsync(initial with
                {
                    Mode = CashFlowAllocationMode.PreviousPeriod
                }));

            await service.SaveCashFlowAllocationStrategyAsync(initial with
            {
                Mode = CashFlowAllocationMode.PreviousPeriod
    ...
    public async Task LegacySettingsMigration_DefaultsModeOnceAndPreservesValues()
    {
        ...
        var store = new SqliteMizanStore(path, false, Today);
        try
        {
            var settings = (await TestFactory.Service(store)
                .GetFinancialPlanAsync()).Settings;

            Assert.Equal(15, settings.IncomeDay);
            Assert.Equal(42_000m, settings.MonthlyVariableExpenseAllowance);
            Assert.Equal(123_000m, settings.ProjectionOpeningBalance);
        ...
        FinancialPlan actual)
    {
        ...
        Assert.Equal(
            expected.PaymentAssignmentStrategies
                .Select(x => (x.Mode, x.EffectiveFromPeriodDate))
                .ToArray(),
    ...
    private static string ProjectionSignature(CashFlowPeriodProjection row) =>
        string.Join("|",
