...
public sealed class ProfileTests
{
    ...
    public async Task ExistingDatabase_BecomesTheFirstProfile_WithItsData()
    {
        await WithRoot(async root =>
        {
            ...
            await using (var legacy = new SqliteCoinFlowStore(legacyPath, true, Today))
            {
            ...
            try
            {
                ...
                var service = TestFactory.Service(store, Today);
                Assert.False(await service.IsOnboardingRequiredAsync());
                Assert.Equal(30_000m, (await store.GetSettingsAsync()).MonthlyLivingBudget);
    ...
    {
        var inner = DispatchProxy.Create<ICoinFlowStore, RecordingStore>();
        var recorder = (RecordingStore)(object)inner;
        var scoped = new ProfileScopedCoinFlowStore(_ => inner);
        await scoped.OpenAsync(Guid.NewGuid());

        foreach (var method in typeof(ICoinFlowStore).GetMethods())
        {
    ...
    private static (ProfileService Profiles, ProfileScopedCoinFlowStore Store, FileSystemProfileRepository Repository)
        Create(string root)
    {
        var repository = new FileSystemProfileRepository(root);
        var store = new ProfileScopedCoinFlowStore(
            id => new SqliteCoinFlowStore(repository.GetDatabasePath(id), true, Today));
    ...
    private static Task WithProfiles(
        Func<ProfileService, ProfileScopedCoinFlowStore, Task> test) =>
        WithRoot(async root =>
        {
