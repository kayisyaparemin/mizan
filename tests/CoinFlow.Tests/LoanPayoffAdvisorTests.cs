...
public sealed class LoanPayoffAdvisorTests
{
    ...
    private static bool Passes(FinancialPlan plan, Loan loan, DateOnly date)
    {
        ...
        var noNewDeficit = result.Rows.All(row =>
            Math.Max(0m, -row.Scenario.EndingProjectedSavings) <=
            Math.Max(0m, -row.Baseline.EndingProjectedSavings) + 0.01m);
        ...
        decimal InPeriods(IReadOnlyList<SalaryPeriodProjection> periods) => periods
        ...
        var gain = result.Scenario[^1].EndingProjectedSavings -
                   result.Baseline[^1].EndingProjectedSavings + beyond;
