...
public sealed class PaymentReminderAnswerTests
{
    ...
    public async Task Answers_PersistAcrossRestart_AreClearedWithData_AndUpgradeFromSchema16AddsTheTable()
    {
        ...
        try
        {
            var answer = new PaymentReminderResponse(
                ...
                Time("2026-09-18T09:02"),
                Time("2026-09-18T12:02"));
            await using (var store = new SqliteCoinFlowStore(path, true, Anchor))
            {
                await TestFactory.Service(store).LoadCanonicalDevelopmentDataAsync();
                await store.UpsertPaymentReminderResponsesAsync([answer]);
            }

            await using (var reopened = new SqliteCoinFlowStore(path, true, Anchor))
            {
            ...
            await using (var upgraded = new SqliteCoinFlowStore(path, true, Anchor))
            {
    ...
    private static async Task<PeriodPlanPaymentLine> LoanLineAsync(SqliteCoinFlowStore store) =>
    ...
    private static async Task WithCanonical(Func<SqliteCoinFlowStore, Task> test)
    {
        var path = TempPath();
        try
        {
